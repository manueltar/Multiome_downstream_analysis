...
volcano_function = function(option_list)
{
  ...
  DA_results<-readRDS(file=opt$DA_results)
  
  cat("DA_results_0\n")
  cat(str(DA_results))
  cat("\n") 
 
  DA_results$Significance<-NA

  DA_results$Significance[which(DA_results$Minus_logpval < 1.3)]<-'NO'
  DA_results$Significance[which(DA_results$Minus_logpval >= 1.3)]<-'YES'

  DA_results$Significance<-factor(DA_results$Significance,
                                             levels=c('NO','YES'),
                                             ordered=T)
  ...
  DA_results_long<-unique(as.data.frame(cSplit(DA_results,sep = ';', direction = "long",
                                                                             splitCols = "Symbol_string"),stringsAsFactors=F))
  
  
  colnames(DA_results_long)[which(colnames(DA_results_long) == 'Symbol_string')]<-'Symbol'
  
  cat("DA_results_long_0\n")
  cat(str(DA_results_long))
  cat("\n")
  
  DA_results_long<-merge(DA_results_long,
                    Gather,
                    by='Symbol',
                    all.x=T)
  
  cat("DA_results_long_1\n")
  cat(str(DA_results_long))
  cat("\n")
  ...
  indx_log2FoldChange<-which(colnames(DA_results_long) == 'log2FoldChange')
  
  A_log2FoldChange<-summary(DA_results_long[,indx_log2FoldChange])
  ...
  indx_Minus_logpval<-which(colnames(DA_results_long) == 'Minus_logpval')
  
  A_Minus_logpval<-summary(DA_results_long[,indx_Minus_logpval])
  ...
  volcano_plot<-ggplot(data=DA_results_long,
                       aes(x=log2FoldChange,
                           y=Minus_logpval)) +
    geom_vline(xintercept=c(-0.25,0.25), color="gray", linetype='dashed',size=0.2)+
    geom_point(data=DA_results_long[which(DA_results_long$Significance == 'NO'),],
               color="black",fill="gray", stroke=0.2, shape=21, size=0.75)+
    geom_point(data=DA_results_long[which(DA_results_long$Significance == 'YES' &
                                           DA_results_long$log2FoldChange >=0.25),],
  ...
  volcano_plot <-volcano_plot+geom_text_repel(data=DA_results_long[which(DA_results_long$Symbol%in%Gather$Symbol &
                                           DA_results_long$Significance == 'YES' &
                                           DA_results_long$log2FoldChange >=0.25),],
                   aes(x=log2FoldChange,
                       y=Minus_logpval,
                       label=Peak_Number,
                   ...
                   box.padding = 1,
                   max.overlaps = Inf)+
    geom_text_repel(data=DA_results_long[which(DA_results_long$Symbol%in%Gather$Symbol &
                                             DA_results_long$Significance == 'YES' &
                                             DA_results_long$log2FoldChange <= -0.25),],
                     aes(x=log2FoldChange,
                         y=Minus_logpval,
                     ...
                     box.padding = 1,
                     max.overlaps = Inf)+
    geom_text_repel(data=DA_results_long[!is.na(DA_results_long$rs) &
                                           (DA_results_long$Significance == 'YES'),],
                     aes(x=log2FoldChange,
                         y=Minus_logpval,
                         label=rs),
...
volcano_subset_function = function(option_list)
{
  ...
  DA_results_subset<-droplevels(DA_results[which(DA_results$seurat_cluster%in%selected_clusters &
                                                   DA_results$comparison%in%selected_comparisons),])
  
  cat("DA_results_subset_0\n")
  cat(str(DA_results_subset))
  cat("\n") 
  
  DA_results_subset$seurat_cluster<-factor(DA_results_subset$seurat_cluster,
                                           levels=selected_clusters,
                                           ordered = T)
  
  DA_results_subset$comparison<-factor(DA_results_subset$comparison,
                                       levels=selected_comparisons,
                                       ordered = T)
  
  cat("DA_results_subset_1\n")
  cat(str(DA_results_subset))
  cat("\n") 
  
  DA_results_subset_long<-unique(as.data.frame(cSplit(DA_results_subset,sep = ';', direction = "long",
                                               splitCols = "Symbol_string"),stringsAsFactors=F))
  
  
  colnames(DA_results_subset_long)[which(colnames(DA_results_subset_long) == 'Symbol_string')]<-'Symbol'
  
  cat("DA_results_subset_long_0\n")
  cat(str(DA_results_subset_long))
  cat("\n")
  
  DA_results_subset_long<-merge(DA_results_subset_long,
                           Gather,
                           all.x=T)
  
  cat("DA_results_subset_long_1\n")
  cat(str(DA_results_subset_long))
  cat("\n")
  ...
  indx_log2FoldChange<-which(colnames(DA_results_subset_long) == 'log2FoldChange')
  
  A_log2FoldChange<-summary(DA_results_subset_long[,indx_log2FoldChange])
  ...
  indx_Minus_logpval<-which(colnames(DA_results_subset_long) == 'Minus_logpval')
  
  A_Minus_logpval<-summary(DA_results_subset_long[,indx_Minus_logpval])
  ...
  volcano_plot<-ggplot(data=DA_results_subset_long,
                       aes(x=log2FoldChange,
                           y=Minus_logpval)) +
    geom_vline(xintercept=c(-0.25,0.25), color="gray", linetype='dashed',size=0.2)+
  ...
  volcano_plot <-volcano_plot+
    geom_text_repel(data=DA_results_subset_long[which(DA_results_subset_long$Symbol%in%Gather$Symbol &
                                                                            DA_results_subset_long$Significance == 'YES' &
                                                                            DA_results_subset_long$log2FoldChange >=0.25),],
                                               aes(x=log2FoldChange,
                                                   y=Minus_logpval,
                                                   label=Peak_Number,
    ...
    geom_text_repel(data=DA_results_subset_long[which(DA_results_subset_long$Symbol%in%Gather$Symbol &
                     ...
                     box.padding = 1,
                     max.overlaps = Inf)+
    geom_text_repel(data=DA_results_subset_long[which(DA_results_subset_long$Symbol == 'CUX1' &
                                                         DA_results_subset_long$Significance == 'YES'),],
                     aes(x=log2FoldChange,
                         y=Minus_logpval,
                         label=Peak_Number),
                    ...
                    xlim=c(breaks_log2FoldChange[1],-0.25),
                     ...
                     box.padding = 1,
                     max.overlaps = Inf)+
    geom_text_repel(data=DA_results_subset_long[!is.na(DA_results_subset_long$rs) &
                                           (DA_results_subset_long$Significance == 'YES'),],
                    aes(x=log2FoldChange,
                        y=Minus_logpval,
                        label=rs),
...
main = function() {
  ...
  option_list <- list(
    ...
    make_option(c("--AKT_signature"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--selected_clusters"), type="numeric", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--selected_comparisons"), type="numeric", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--DA_results"), type="numeric", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
