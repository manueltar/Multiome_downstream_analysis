...
collects_GeneEXP = function(option_list)
{
  ...
  {
    cluster_group_array_sel<-cluster_array[i]
    
    cat("--------comparison-------------->\t")
    cat(sprintf(as.character(cluster_group_array_sel)))
    cat("\t")
    

    path_comparisons_cluster_group<-paste(out,cluster_group_array_sel,'/',sep='')
    
    cat(sprintf(as.character(path_comparisons_cluster_group)))
    cat("\n")
    
   
    #### Open Normalised_counts file ----
    
    if (file.exists(path_comparisons_cluster_group)){
      
      setwd(path_comparisons_cluster_group)
      ...
      if(dim(comparisons_df)[1] >0)
      {
        ...
        comparisons_df.dt<-data.table(comparisons_df, key = c('Symbol','sample_id','seurat_cluster'))
    ...
    }#file.exists(path_comparisons_cluster_group)
  ...
  if(length(list_results) >0)
  {
    ...
    FINAL_df<-FINAL_df[order(FINAL_df$time_point,FINAL_df$seurat_cluster,FINAL_df$clone_line,FINAL_df$Symbol),]
    
    cat("FINAL_df_0\n")
    cat(str(FINAL_df))
    cat("\n")
    cat(str(unique(FINAL_df$Symbol)))
    cat("\n")
    ...
    metadata<-unique(FINAL_df[,-c(which(colnames(FINAL_df) == 'count'),
                                  which(colnames(FINAL_df) == 'Symbol'))])
