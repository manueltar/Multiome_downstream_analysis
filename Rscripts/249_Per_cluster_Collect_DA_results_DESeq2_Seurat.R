...
collects_DA = function(option_list)
{
  opt_in = option_list
  opt <<- option_list
  ...
  for(i in 1:length(cluster_array))
  {
    ...
    #### Open DA_results file ----
    
    if (file.exists(path_comparisons_seurat_cluster)){
      setwd(path_comparisons_seurat_cluster)
      
      filename<-paste("DA_peaks",'.tsv',sep='')
      
      if (file.exists(filename)){
        
        DA_results<-as.data.frame(fread(file=filename, sep="\t", header=T), stringsAsFactors=F)
        ...
        if(dim(DA_results)[1] >0)
        {
          

          DA_results$seurat_cluster<-cluster_array_sel
          ...
          if(DEBUG == 1)
          {
            ...
            cat(str(DA_results))
            cat("\n")
            cat(sprintf(as.character(names(summary(as.factor((unique(DA_results$comparison))))))))
            cat("\n")
            cat(sprintf(as.character(summary(as.factor((unique(DA_results$comparison)))))))
            cat("\n")
          }
          
          list_results[[i]]<-DA_results
  ...
  if(length(list_results) >0)
  {
    ...
    FINAL_df<-merge(FINAL_df,
                    Master_peak_file_with_SNP_numbered_peaks,
                    by='Peak_ID')
    ...
    Summary_table_DA <- as.data.frame(FINAL_df.dt[Minus_logpval >=1.3,.(n_DE_genes=.N), by = key(FINAL_df.dt)])
    
    
    
    cat("Summary_table_DA_0\n")
    cat(str(Summary_table_DA))
    cat("\n")
    
    Summary_table_DA_UP <- as.data.frame(FINAL_df.dt[Minus_logpval >=1.3 & log2FoldChange >=0.25,.(n_DE_genes_UP=.N), by = key(FINAL_df.dt)])
    
    
    
    cat("Summary_table_DA_0\n")
    cat(str(Summary_table_DA_UP))
    cat("\n")
    
    
    Summary_table_DA_DOWN <- as.data.frame(FINAL_df.dt[Minus_logpval >=1.3 & log2FoldChange <= -0.25,.(n_DE_genes_DOWN=.N), by = key(FINAL_df.dt)])
    
    
    
    cat("Summary_table_DA_0\n")
    cat(str(Summary_table_DA_DOWN))
    cat("\n")
    ...
    write.table(FINAL_df,file="DA_results.tsv", sep="\t", quote=F, row.names = F)
    saveRDS(FINAL_df,file="DA_results.rds")
    
    write.table(Summary_table,file="Summary_table_DA.tsv", sep="\t", quote=F, row.names = F)
