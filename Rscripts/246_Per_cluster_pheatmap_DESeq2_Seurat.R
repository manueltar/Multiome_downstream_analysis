...
heatmap_WT = function(option_list)
{
  ...
  path_graphs = paste(out,'GeneEXP_heatmaps','/', sep='')
  ...
  #### READ and transform original_selection_of_genes ----
  
  original_selection_of_genes = unlist(strsplit(opt$original_selection_of_genes, split=','))
  
  cat("original_selection_of_genes_\n")
  cat(sprintf(as.character(original_selection_of_genes)))
  cat("\n")
  ...
  new_genes_to_add$GENE_CLASS[which(new_genes_to_add$Symbol%in%original_selection_of_genes)]<-'REST_of_genes'
  ...
  cat("\n")
  cat(str(sprintf(as.character(names(summary(as.factor(new_genes_to_add$GENE_CLASS)))))))
  cat("\n")
  cat(str(sprintf(as.character(summary(as.factor(new_genes_to_add$GENE_CLASS))))))
  cat("\n")
  ...
  #### READ and transform GeneEXP_matrix ----
  
  df_for_matrix<-as.data.frame(fread(file=opt$GeneEXP_matrix, sep="\t", header=T), stringsAsFactors=F)
  ...
  lcpm <- cpm(GeneEXP_matrix, log=TRUE)
  ...
  lcpm_subset<-lcpm[which(row.names(lcpm)%in%ALL_genes$Symbol),]
  ...
  annotation_row = data.frame(GeneClass = ALL_genes$GENE_CLASS)
  
  rownames(annotation_row) = ALL_genes$Symbol
  ...
  cat("levels(metadata$Genotype)\n")
  cat(sprintf(as.character(levels(metadata$Genotype))))
  cat("\n")
  ...
  cat("KEY_FINAL_row.names(annotation_col)\n")
  cat(sprintf(as.character(row.names(annotation_col))))
  cat("\n")
  ...
  ann_colors = list(
    ...
    GeneClass = c('CUX1' = 'black',
                  ...
                  'AKT_progenitor' = brewer.pal(8, "BrBG")[1],
  ...
  cat(str(ann_colors))
  cat("\n")
  cat(sprintf(as.character(names(ann_colors$clone_line))))
  cat("\n")
  cat("\n")
  cat(sprintf(as.character(names(ann_colors$seurat_cluster))))
  cat("\n")
  cat("\n")
  cat(sprintf(as.character(names(ann_colors$Genotype))))
  cat("\n")
  cat("\n")
  cat(sprintf(as.character(names(ann_colors$time_point))))
  cat("\n")
  cat("\n")
  cat(sprintf(as.character(names(ann_colors$GeneClass))))
  cat("\n")
  cat("\n")
  ...
  svgname<-paste(paste("Heatmap","WT", sep='_'),".svg",sep='')
...
heatmap_ALL = function(option_list)
{
  ...
  # colnames(lcpm)<-gsub("chrGFP_","",colnames(lcpm))
  # colnames(lcpm)<-gsub("KI_","KI",colnames(lcpm))
  # colnames(lcpm)<-gsub("Del_","Del",colnames(lcpm))
  ...
  svgname<-paste(paste("Heatmap","ALL_genotypes", sep='_'),".svg",sep='')
...
main = function() {
  ...
  option_list <- list(
    ...
    make_option(c("--ery_genes_array"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--original_selection_of_genes"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    ...
    make_option(c("--marker_genes_array"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--GeneEXP_matrix"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
