...
data_wrangling = function(option_list)
{
  ...
  Master_peak_file_with_SNP<-readRDS(file=opt$Master_peak_file_with_SNP)
  
  
  cat("Master_peak_file_with_SNP\n")
  cat(str(Master_peak_file_with_SNP))
  cat("\n")
  ...
  #### Select ATAC layer and create a new assay only for the selected peaks
  
  # Reduce peak set to the peaks in the master file
  ...
  Peak_list_filt=Peak_list %>% dplyr::filter(peak_ids %in% Master_peak_file_with_SNP$Peak_ID)
  ...
  Seurat_object[["peak_subset"]] <- CreateChromatinAssay(
    counts = peak_subset,
    fragments = Fragments(Seurat_object),
    min.cells = 0, min.features = -1 )
  
  DefaultAssay(Seurat_object) <- 'peak_subset'
  
  
  
  #### Extract peak_subset counts ----
  
  matrix_ATAC<-GetAssayData(object = Seurat_object, assay = "peak_subset", layer = "counts")
...
#### LOOP FOR COMPARISON OF GENOTYPES ------
  ...
  for(i in 2:length(array_Genotypes))
  {
    ...
    colnames(res_LRT_tb)[which(colnames(res_LRT_tb) == 'gene')]<-'Peak_ID'
    ...
    array_Peaks<-row.names(dds_lrt@assays@data@listData$counts)
    ...
    cat(paste(c('count','Genotype','time','sample_id','time_point','cluster_id','Assigned_GFPbc','Peak_ID'), collapse="\t"),
        file=filename,sep="\n")
    
    ALL_counts<-data.frame()
    
    for(k in 1:length(array_Peaks))
    {
      array_genes_sel<-array_Peaks[k]
  ...
  if(dim(Results_genotype_comparisons)[1] > 0)
  {
    ...
    write.table(Results_genotype_comparisons,file="DA_peaks.tsv",sep="\t",          
                quote = FALSE, 
                row.names = FALSE)
...
main = function() {
  ...
  option_list <- list(
    ...
    make_option(c("--Master_peak_file_with_SNP"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
